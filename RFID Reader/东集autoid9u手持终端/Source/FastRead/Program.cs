...
namespace FastRead
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            #region 读
            Console.WriteLine("Start Fast Read");
            ...
            for (int i = 1; i <= 10; i++)
            {
                ...
                Tag tagData =null;
                result = jwReader.RFID_Fast_Read(accessParam, out tagData);
                if (result == Result.OK)
                    Console.WriteLine(string.Format("Count={0},EPC={1},DATA={2}", i, tagData.EPC,tagData.DATA));
                else
                    Console.WriteLine("Fast Read Failure");
            }
            Console.WriteLine("Stop Fast Read");
            #endregion
